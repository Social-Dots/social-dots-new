...
class Command(BaseCommand):
    help = 'Format all blog post content with inline styles'
        ...
        for post in BlogPost.objects.all():
            if post.content:
                ...
                if formatted_content != original_content:
                    ...
                    formatted_count += 1
                    self.stdout.write(
                        self.style.SUCCESS(f'Formatted blog post: {post.title}')
                    )
        
        self.stdout.write(
            self.style.SUCCESS(f'Successfully formatted {formatted_count} blog posts')
        )